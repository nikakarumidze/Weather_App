...
document
  .querySelector('#carouselFirst .carousel-control-next')
  .addEventListener('click', (e) => {
    // Finds Where we are now
    let carouselRowNum = document
    ...
    document
      .querySelector('#carousel_second_content .active')
      .classList.remove('active');

    // Change Slide
    if (
      carouselRowNum <
      document.querySelector('#carouselFirst .carousel-indicators')
        .childElementCount
    ) {
      weatherContent.children.item(carouselRowNum).classList.add('active');
      document
        .querySelector('#carouselFirst .carousel-indicators')
        .children.item(carouselRowNum)
...
document
  .querySelector('#carouselFirst .carousel-control-prev')
  .addEventListener('click', (e) => {
    let carouselRowNum = document
      .querySelector('#carouselFirst .carousel-indicators .active')
      .getAttribute('data-bs-slide-to');
    ...
    if (carouselRowNum < 2) {
      weatherContent.children
        .item(weatherContent.children.length - 1)
        .classList.add('active');
      document
        .querySelector('#carouselFirst .carousel-indicators')
        .children.item(
          document.querySelector('#carouselFirst .carousel-indicators').children
            .length - 1
        )
        .classList.add('active');
      ...
      document
        .querySelector('#carouselFirst .carousel-indicators')
        .children.item(carouselRowNum - 2)
        .classList.add('active');
