...
function updateUI(data, location, active, activeTrue) {
  ...
  if (active == 'active') {
    if (
      document.querySelector('#carouselFirst .carousel-indicators .active') !==
      null
    ) {
      document
...
function dailyUI(data, active) {
  ...
  if (localStorage.getItem('metric') == 'F') {
    ...
    metricScore2 =
      Math.round(
        (((data.DailyForecasts[0].Temperature.Minimum.Value - 32) * 5) / 9) * 10
      ) / 10;
    metricScore3 =
      Math.round(
        (((data.DailyForecasts[0].Temperature.Maximum.Value - 32) * 5) / 9) * 10
      ) / 10;
  }
